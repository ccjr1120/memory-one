...
registerTool("memory_update", "Use only when the user explicitly asks to correct, refine, or supersede a stored memory. Fetch the record first when needed, then update only the changed fields. Scope is an optional category.", { memory_id: z.string(), patch: z.record(z.string(), z.unknown()) }, async ({ memory_id, patch }: any) => trackToolCall("memory_update", () => json(exposeMemory(store.update(memory_id, toStoragePatch(patch as Record<string, unknown>)) ?? { error: "memory_not_found" }))));
