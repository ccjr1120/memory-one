...
把这段配置放进客户端的 MCP 设置后，重启客户端或新开一个会话。服务启动时会自动创建一个拥有全部工具权限的默认 Key；平台 Agent、Codex 和其他客户端建议分别创建独立 Key，并按最小权限选择工具。新建 Key 的明文只在创建成功后显示一次。Memory One 默认只监听本机；如果要暴露到其他设备，请同时配置网络层和 Key 管理。
