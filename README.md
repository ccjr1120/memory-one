...
1. 打开工作台右下角的 **记忆管家**。
2. 用自然语言让它保存一条希望 Agent 长期遵守的偏好或项目约定。
3. 在你的 Agent 中开始一个新任务，确认它调用了 `memory-get-context`。
4. 回到 **MCP 服务** 页面查看调用统计，或在 **全部记忆** 中检查新增和召回的记忆。

## 主要能力

- **长期保存经验**：记录偏好、事实、项目决策、工作流程和纠正意见。
- **任务前自动召回**：每个新任务的首次消息通过 `memory-get-context` 读取固定上下文，同一任务后续通过 `memory-search` 按需检索其他记忆。
- **区分项目与通用记忆**：使用可选的 `scope` 对记忆分类，同时支持项目记忆与全局记忆联合召回。
- **重复与替代保护**：完全重复不会再次写入；相似内容会作为候选返回，替代旧经验需要显式指定。
- **可视化管理**：搜索、筛选和查看记忆，通过记忆管家创建、编辑或删除记忆，并可查看召回次数与置信度。
- **召回可诊断**：查看单条记忆和全局最近召回的来源、查询和时间。
- **安全备份**：从设置页导出 JSON，或按需合并/替换恢复。
...
5. 按需设置默认 Scope，以及是否在每个新任务的首次消息前自动读取上下文。
6. 保存配置后开始对话。

模型配置保存在本机 SQLite 数据库中；记忆操作仍通过 Memory One 的 MCP 工具完成。内置 Agent 只有在用户明确要求记住、保存、修改或删除时才会写入记忆，未明确要求时会先询问确认。
...
推荐使用 Web 界面备份：进入 **设置 → 数据备份**，选择 **导出 JSON**。恢复时可以选择：

- **合并导入**：跳过完全重复的记忆，保留当前数据。
- **替换导入**：确认后删除当前记忆，并使用备份中的记忆和安全配置替代。
...
然后复制 `memory.db` 文件。恢复时，在服务停止状态下用备份文件替换原数据库，再重新启动服务。升级到 schema v2 时，服务会在数据库目录的 `backups/` 中自动创建迁移前备份。
